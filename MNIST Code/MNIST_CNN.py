...
def load_data(
    ...
    if (name=="MNIST"):
         ...
         train_valid_set =  torchvision.datasets.CIFAR10(
               root=data_path, train=True, download=True, transform=transform )
          # Load test set
...
def get_partition_data_loaders(
    ...
    # pyre-ignore [6]
    print("Downsample", downsample_pct)
    downsampled_num_examples = int(downsample_pct * len(train_valid_set))
    print("train_valid",len(train_valid_set))
    ...
    print("subset_of_train_samples", len(train_set))
    print("subset_of_valid_samples", len(valid_set))
    print("subset_of_test_samples", len(test_set))
    ...
    print("number_of_train_batches", len(train_loader))
    print("number_of_valid_batches", len(valid_loader))
    print("number_of_test_batches", len(test_loader))
...
def train(
    ...
    optimizer = optim.SGD(
        ...
        weight_decay=parameters.get("weight_decay", 0.005),
    )
  ...
  #  num_epochs = parameters.get("num_epochs", 1)
    ...
    for _ in range(num_epochs):
        ...
        loss_calc.append(round(train_loss,4))
          # scheduler.step()
    return (net,loss_calc)
...
def evaluate(
    ...
    with torch.no_grad():
        for inputs, labels in data_loader:
            ...
            labels = labels.to(device=device)
            scores = net(inputs)
            _, predictions = torch.max(scores.data, 1)  # Get the index of the max log-probability
            num_correct += (predictions == labels).sum()  # Count correct predictions
